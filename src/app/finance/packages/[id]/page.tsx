...
function SummaryPanel({
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Validation */}
      <div className={`rounded-xl border p-4 ${pkg.isReadyToSell && revOk ? 'bg-emerald-50 border-emerald-200' : 'bg-amber-50 border-amber-200'}`}>
        <div className={`flex items-center gap-2 text-xs font-semibold mb-1 ${pkg.isReadyToSell && revOk ? 'text-emerald-700' : 'text-amber-700'}`}>
          {pkg.isReadyToSell && revOk ? (
            <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
          ) : (
            <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" /></svg>
          )}
          {pkg.isReadyToSell && revOk ? 'Package ready to sell' : 'Needs attention'}
        </div>
        {!revOk && (
          <p className="text-xs text-amber-600">Revenue recognition must total 100% (currently {revTotal}%)</p>
        )}
        {!pkg.isReadyToSell && (
          <p className="text-xs text-amber-600 mt-0.5">Unresolved content validation issues</p>
        )}
      </div>
...
export default function PackageCockpitPage() {
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-gray-200">
        ...
        <div className="flex items-center gap-3 px-6 py-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-xs text-gray-400">{pkg.trade}</span>
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${statusText[pkg.status]}`}>
              <span className={`w-1.5 h-1.5 rounded-full ${statusDot[pkg.status]}`} />
              {pkg.status}
            </span>
          </div>
        </div>
      ...
      {/* Validation alert */}
      {pkg.isReadyToSell && revTotal === 100 ? (
        <div className="flex items-center gap-2.5 px-6 py-2.5 bg-emerald-50 border-b border-emerald-100 text-xs text-emerald-700">
          ...
          <span><strong>Package is ready to sell</strong> — all validation checks passed.</span>
        </div>
      ) : (
        <div className="flex items-start gap-2.5 px-6 py-2.5 bg-amber-50 border-b border-amber-100 text-xs text-amber-700">
          <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} className="shrink-0 mt-0.5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
          </svg>
          <span>
            {!pkg.isReadyToSell && <strong>Package has outstanding validation issues. </strong>}
            {revTotal !== 100 && `Revenue recognition totals ${revTotal}% — adjust stage percentages to reach 100%.`}
          </span>
        </div>
      )}
